        ...
        async def wrapper(*args, **kwargs):
            ...
            while remaining > 0:
                try:
                    ...
                    exceptions
                ) as exc:  # noqa: PLW0703 - deliberate catch for retry logic
                    attempts += 1
                    last_exception = exc
                    ...
                    if metadata.get("reason") == "executor_unavailable":
                        logger.warning(
                            f"Executor unavailable in {func.__name__}. Not retrying."
                        )
                        raise exc

                    if status_code == 404:
                        logger.warning(
                            f"Resource not found (404) in {func.__name__}. Not retrying."
                        )
                        raise exc
                    ...
                    if (
                        isinstance(
                            exc,
                            (MediaUploadError, MediaDownloadError, MediaServerError),
                        )
                        and metadata.get("status_code") == 401
                    ):
                        logger.error(
                            f"Authentication error (401) in {func.__name__} - not retrying"
                        )
                        raise exc
                    ...
                    if isinstance(exc, requests.HTTPError) and status_code == 429:
                        ...
                        wait_time = _retry_after_from_response(
                            exc.response, float(fallback)
                        )
                        header_value = (
            ...
            logger.error(
                f"Maximum retries reached for {func.__name__}. Last error: {last_exception}"
            )
            if last_exception:
            ...
            raise MediaServerError(
                f"Failed after {max_tries} tries without specific exception"
            )
...
class MediaManager:
    def __init__(self, api_base_url: str, media_folder: str):
        ...
        self.rate_limiter = RateLimiter(
            MAX_REQUESTS_PER_MINUTE, REQUEST_TRACKING_WINDOW
        )
        self.upload_rate_limiter = RateLimiter(
            UPLOAD_REQUESTS_PER_MINUTE, REQUEST_TRACKING_WINDOW
        )
    ...
    async def _get_semaphore(self) -> asyncio.Semaphore:
        ...
        if self.semaphore is None:
            async with self._semaphore_lock:
                ...
                if self.semaphore is None:
                    logger.debug(
                        f"Initializing asyncio.Semaphore for loop {id(current_loop)}"
                    )
    ...
    async def _get_upload_semaphore(self) -> asyncio.Semaphore:
        ...
        try:
            ...
            raise RuntimeError(
                "Cannot get upload semaphore without a running event loop"
            )

        if (
            self._upload_semaphore_lock is None
            or self._upload_semaphore_loop is not current_loop
        ):
        ...
        if self.upload_semaphore is None:
            async with self._upload_semaphore_lock:
                if self.upload_semaphore is None:
                    logger.debug(
                        f"Initializing upload asyncio.Semaphore for loop {id(current_loop)}"
                    )
                    self.upload_semaphore = asyncio.Semaphore(
                        self.upload_semaphore_size
                    )
    ...
    @staticmethod
    def _raise_with_metadata(
        exc: Exception, context: Dict, *, cause: Optional[BaseException] = None
    ) -> NoReturn:
    ...
    def _handle_upload_http_error(
        self, exc: requests.HTTPError, base_context: Dict
    ) -> NoReturn:
        status_code = getattr(exc.response, "status_code", None)
        error_text = getattr(exc.response, "text", str(exc))
        logger.error(
            f"HTTP error {status_code} uploading media ({base_context.get('filename')}): {error_text}"
        )
        error_context = {
            **base_context,
            "status_code": status_code,
            "response_text": (
                error_text[:512] if isinstance(error_text, str) else None
            ),
        }

        if status_code == 403:
            media_error: MediaError = MediaUploadError(
                f"Permission denied (403): {error_text}"
            )
            ...
            if retry_after_header:
                try:
                    ...
                    logger.warning(
                        f"Invalid Retry-After header value: {retry_after_header}"
                    )
        else:
            media_error = MediaUploadError(
                f"Media upload failed with HTTP {status_code}: {error_text}"
            )
    ...
    def _ensure_executor_available(self):
        ...
        if needs_recreation:
            ...
            if not self._is_anki_available():
                logger.info(
                    "Anki is closing, not recreating executor - aborting operation"
                )
    ...
    async def async_request(self, method, url, **kwargs):
        ...
        def do_request():
            try:
                ...
                logger.error(
                    f"Network request failed: {method.upper()} {url} -> {req_exc}"
                )
    ...
    async def _calculate_file_hash(self, filepath: Path) -> str:
        ...
        def calculate_hash():
            ...
            try:
                ...
                raise MediaHashError(
                    f"IOError reading file for hashing {filepath}: {io_err}"
                ) from io_err
    ...
    async def compute_file_hash_and_size(
        self, filepath: Union[str, Path], *, bypass_cache: bool = False
    ) -> Tuple[str, int]:
        ...
        if not bypass_cache:
            try:
                ...
                if (
                    cache_key in self.hash_cache
                    and self.hash_cache[cache_key][0] == mtime
                ):
                    return (self.hash_cache[cache_key][1], file_size)
            except OSError as e:
                # Log error but proceed to calculate hash if possible
                logger.warning(
                    f"Could not get mtime for hash cache check: {filepath} - {e}"
                )
            except KeyError:
        ...
        try:
            ...
            try:
                ...
                logger.warning(
                    f"File {filepath} possibly deleted during hash calculation."
                )
            ...
            raise MediaHashError(
                f"Unexpected error computing file hash for {filepath}: {str(e)}"
            ) from e
    ...
    async def upload_file(
        ...
        if not self.validate_file_basic(filepath):
            ...
            self._raise_with_metadata(
                exc, {**base_context, "reason": "validation_failed"}
            )
        ...
        async with semaphore:
            try:
                    ...
                    try:
                        with open(filepath, "rb") as f:
                            ...
                            if len(file_content) != filepath.stat().st_size:
                                raise IOError(
                                    f"File size changed during read: {filepath}"
                                )
                ...
                try:
                    ...
                    logger.error(
                        "upload_file (do_upload) called without a running event loop!"
                    )
                    raise RuntimeError(
                        "No running event loop found for file upload"
                    ) from exc
                ...
                if isinstance(e, (IOError, MediaHashError)):
                    logger.error(
                        f"I/O or Hash error preparing upload for {filepath}: {e}"
                    )
                    self._raise_wrapped_error(
                        MediaUploadError,
                    ...
                    logger.exception(
                        f"Unexpected error during upload_file for {filepath}: {e}"
                    )
                    self._raise_wrapped_error(
                        MediaUploadError,
    ...
    async def download_file(self, url: str, destination: Union[str, Path]) -> bool:
        ...
        temp_destination = destination.with_suffix(
            f"{destination.suffix}.tmp.{uuid.uuid4().hex[:8]}"
        )
        ...
        try:
            ...
            try:
                ...
                self._raise_with_metadata(
                    error, {**base_context, "reason": "executor_unavailable"}
                )
            ...
            def do_download():
                ...
                try:
                    with self.session.get(
                        url, stream=True, timeout=REQUEST_TIMEOUT * 2, verify=VERIFY_SSL
                    ) as response:  # Longer timeout for download, verify SSL
                        response.raise_for_status()
                    ...
                    logger.error(
                        f"IOError writing temporary download file {temp_destination}: {io_err}"
                    )
                    raise io_err
            ...
            try:
                ...
                raise RuntimeError(
                    "No running event loop found for file download"
                ) from exc
            ...
            if success and temp_destination.exists():
                try:
                    ...
                    if self._file_exists_with_size(destination):
                        logger.debug(
                            f"Destination {destination} already exists, discarding downloaded temp file"
                        )
                    ...
                    if self._file_exists_with_size(destination):
                        logger.debug(
                            f"Destination {destination} created by another download during rename, discarding temp file"
                        )
                    ...
                    try:
                        ...
                        if self._file_exists_with_size(destination):
                            logger.debug(
                                f"Destination {destination} exists after rename failure, assuming success"
                            )
                            temp_file_renamed = True
                            return True
                        logger.error(
                            f"Failed to rename temp file {temp_destination} to {destination}: {rename_err2}"
                        )
                        return False
                return True
            else:
                ...
                self._raise_with_metadata(
                    error, {**base_context, "reason": "missing_temp_file"}
                )
            ...
            if not temp_file_renamed and temp_destination.exists():
                try:
                    ...
                    logger.warning(
                        f"Could not delete temporary download file {temp_destination}: {unlink_err}"
                    )
        ...
        async with semaphore:
            ...
            try:
                ...
                if status == 401:
                    ...
                    exc = MediaServerError(
                        f"Server error {status} getting manifest: {text}"
                    )
                context = {
                    **base_context,
                    "status_code": status,
                ...
                ValueError,
                KeyError,
            ) as e:  # Added ValueError/KeyError for bad JSON
                logger.error(f"Error retrieving or parsing manifest data: {str(e)}")
                exc = MediaServerError(
                    f"Failed to get or parse server response for media manifest: {str(e)}"
                )
                context = {
                    "stage": "get_media_manifest",
                    "deck_hash": deck_hash,
    ...
    async def get_media_manifest_and_download(
        self, deck_hash: str, filenames: List[str], progress_callback=None
    ) -> Dict:
        if not mw.col:
            return {
                "success": False,
        ...
        if duplicates_removed > 0:
            logger.info(
                f"Removed {duplicates_removed} duplicate filename(s) from download list"
            )
        ...
        if total_files == 0:
            return {
                "success": True,
                "message": "No files to download after deduplication",
                "downloaded": 0,
                "skipped": 0,
                "failed": 0,
            }
        ...
        for i in range(0, total_files, manifest_batch_size):
            ...
            try:
                manifest_data = await self.get_media_manifest(
                    deck_hash, batch_filenames
                )
                if manifest_data and "files" in manifest_data:
                    ...
                    if files_in_batch:
                        ...
                        logger.warning(
                            f"Manifest batch {i // manifest_batch_size + 1} returned no files."
                        )
                else:
                    logger.warning(
                        f"Invalid manifest format or no files found in batch {i // manifest_batch_size + 1}."
                    )
            except MediaServerError as e:
                logger.error(
                    f"Failed to get manifest batch {i // manifest_batch_size + 1}: {e}"
                )
                ...
                return {
                    "success": False,
                    "message": f"Failed to get manifest: {e}",
                    "downloaded": 0,
                    "skipped": 0,
                    "failed": total_files,
                }
                ...
                logger.exception(
                    f"Unexpected error getting manifest batch {i // manifest_batch_size + 1}: {e}"
                )
                ...
                return {
                    "success": False,
                    "message": f"Unexpected error getting manifest: {e}",
                    "downloaded": 0,
                    "skipped": 0,
                    "failed": total_files,
                }
        ...
        for file_info in all_files_to_download:
            ...
            if filename:
                seen_filenames[filename] = (
                    file_info  # Overwrites earlier entries with same filename
                )
        ...
        if duplicates_in_manifest > 0:
            logger.info(
                f"Removed {duplicates_in_manifest} duplicate file(s) from manifest response"
            )

        # Phase 2: Download files
        num_to_download = len(all_files_to_download)
        logger.info(
            f"Manifest retrieved. Attempting to download {num_to_download} files."
        )
        if num_to_download == 0:
            ...
            return {
                "success": True,
                "message": "No files needed downloading based on manifest.",
                "downloaded": 0,
                "skipped": total_files,
                "failed": 0,
            }
        ...
        for i in range(0, num_to_download, download_batch_size):
            ...
            seen_in_batch = (
                set()
            )  # Track filenames in current batch to avoid concurrent downloads
            ...
            for file_info in download_batch:
                ...
                if not filename or not download_url:
                    logger.warning(
                        f"Invalid file info in manifest, skipping: {file_info}"
                    )
                    total_failed += 1
                    continue
                ...
                if filename in seen_in_batch:
                    logger.warning(
                        f"Duplicate filename {filename} within download batch, skipping duplicate"
                    )
                    total_skipped += 1
                    continue
                ...
                try:
                    ...
                    logger.warning(
                        f"Path traversal detected in filename '{filename}', skipping"
                    )
                    total_failed += 1
                    continue
                except (OSError, RuntimeError):
                ...
                for idx, result in enumerate(results):
                    ...
                    if isinstance(result, Exception):
                        ...
                        if reason == "executor_unavailable":
                            logger.info(
                                f"Executor unavailable during download of {task_filename}; aborting remaining downloads."
                            )
                            return {
                                "success": False,
                                "message": f"Download aborted due to shutdown",
                        ...
                        logger.error(
                            f"Exception downloading file {task_filename}: {result}"
                        )
                        exception_context = {
                            ...
                            "download_url": (
                                download_batch[idx].get("download_url")
                                if idx < len(download_batch)
                                else None
                            ),
                        }
                        if isinstance(metadata, dict):
                        ...
                        logger.error(
                            f"Download function returned False for file {task_filename}"
                        )
                        capture_media_message(
            ...
            if progress_callback:
                ...
                progress_value = (
                    processed_count / num_to_download if num_to_download > 0 else 1.0
                )
        ...
        logger.info(
            f"Download complete. Downloaded: {total_downloaded}, Skipped: {total_skipped}, Failed: {total_failed}"
        )
        return {
    ...
    @retry(max_tries=3, delay=1)
    async def check_media_bulk(
        self, deck_hash: str, bulk_operation_id: str, files: List[Dict]
    ) -> Dict:
        if not files:
            return {
                "existing_files": [],
        ...
        async with semaphore:
            ...
            for f in files:
                if all(k in f for k in ("hash", "filename", "note_guid", "file_size")):
                    ...
                    logger.warning(
                        f"Skipping invalid file info in check_media_bulk: {f}"
                    )
            ...
            try:
                ...
                return {
                    ...
                    "failed_files": json_response.get(
                        "failed_files", []
                    ),  # Add failed files key
                ...
                exc = MediaServerError(
                    f"Network or parsing error checking media: {str(e)}"
                )
                context = {
                    "stage": "check_media_bulk",
                    "deck_hash": deck_hash,
    ...
    async def optimize_media_for_upload(
        self, file_note_pairs: List[Tuple[str, str]], progress_callback=None
        ...
        logger.info(
            f"Categorization complete: {len(svg_files)} SVGs, {len(regular_files)} regular files"
        )
        ...
        if svg_files:
            ...
            try:
                svg_optimized = await media_optimizer.optimize_svg_files(
                    svg_optimize_list
                )
            except Exception as e:
                ...
                if filename in svg_optimized:
                    ...
                    if was_optimized and reported_hash:
                        try:
                            ...
                            disk_hash, file_size = (
                                await self.compute_file_hash_and_size(
                                    opt_filepath, bypass_cache=True
                                )
                            )
                            ...
                            logger.error(
                                f"Error processing optimized SVG {filename}: {e}"
                            )
        ...
        if regular_files:
            ...
            for i in range(0, len(regular_files), batch_size):
                ...
                for filename, filepath_obj, note_guid in batch:
                    task = self._process_regular_file(
                        filename, filepath_obj, note_guid, media_optimizer
                    )
                ...
                for idx, result in enumerate(batch_results):
                    ...
                    if not isinstance(result, tuple) or len(result) != 3:
                        logger.error(
                            f"Invalid result format for file {filename}: {result}"
                        )
                        continue
        ...
        logger.info(
            f"Media optimization complete. Mapping: {len(filename_mapping)}, Files to check: {len(files_info)}"
        )
        if progress_callback:
    ...
    async def _process_regular_file(
        self, filename: str, filepath_obj: Path, note_guid: str, media_optimizer
    ) -> Optional[Tuple]:
        ...
        try:
            ...
            try:
                ...
                if (
                    ...
                    and len(cached_entry) > 4
                    and cached_entry[4] == file_size
                ):
                    ...
                    if (
                        opt_filepath is None
                        or current_filename is None
                        or not opt_filepath
                        or not current_filename
                    ):
                        logger.warning(
                            f"Invalid cached data for {filename}, re-optimizing (opt_filepath={opt_filepath}, current_filename={current_filename})"
                        )
                        ...
                        file_hash, file_size = await self.compute_file_hash_and_size(
                            opt_filepath
                        )

                        if file_size > MAX_FILE_SIZE:
                            logger.warning(
                                f"Cached file (optimized: {was_optimized}) exceeds size limit: {opt_filepath} ({file_size} bytes)"
                            )
                            return None
                        ...
                        logger.debug(
                            f"Cached file no longer exists for {filename}, re-optimizing"
                        )
            except OSError:
            ...
            opt_filepath, current_filename, was_optimized = (
                await media_optimizer.optimize_media_file(filename, filepath_obj)
            )
            ...
            if (
                ...
                logger.warning(
                    f"Optimization failed for file {filename}, skipping (opt_filepath={opt_filepath}, current_filename={current_filename})"
                )
                return None
            ...
            if file_size > MAX_FILE_SIZE:
                logger.warning(
                    f"File (optimized: {was_optimized}) exceeds size limit: {opt_filepath} ({file_size} bytes)"
                )
                return None
    ...
    async def upload_media_bulk(
        ...
        try:
            ...
            bulk_check_result = await self.check_media_bulk(
                deck_hash, bulk_operation_id, files_info
            )
            ...
            check_failed_count = (
                len(check_failed_hashes)
                if check_failed_hashes
                else len(check_failed_files)
            )
            if check_failed_count:
                logger.warning(
                    f"{check_failed_count} files failed server-side check: {check_failed_files}"
                )
            ...
            logger.info(
                f"Check complete. Existing: {existing_count}, Failed Check: {check_failed_count}, To Upload: {len(missing_files_to_upload)}"
            )
            if progress_callback:
            ...
            if not batch_id:
                ...
                return {
                    "success": False,
                    "status": "no_batch_id",
                    "message": "Server rejected upload batch.",
                    "uploaded": 0,
                    "existing": existing_count,
                    "failed": len(failed_hashes_set),
                    "failed_filenames": sorted(failed_filenames_set),
                }
        except MediaServerError as e:
            ...
            if metadata.get("status_code") == 401:
                ...
                return {
                    ...
                    "uploaded": 0,
                    "existing": 0,
                    "failed": total_initial_files,
                    "failed_filenames": list(hash_to_filename.values()),
                }
            capture_media_exception(
                e,
                context={
            ...
            return {
                "success": False,
                "status": "check_failed",
                "message": f"Server error checking files: {e}",
                "uploaded": 0,
                "existing": 0,
                "failed": total_initial_files,
                "failed_filenames": list(hash_to_filename.values()),
            ...
            return {
                "success": False,
                "status": "check_failed_unexpected",
                "message": f"Unexpected error checking files: {e}",
                "uploaded": 0,
                "existing": 0,
                "failed": total_initial_files,
                "failed_filenames": list(hash_to_filename.values()),
        ...
        upload_delay = (
            UPLOAD_BATCH_DELAY  # Delay between batches to respect rate limits
        )
        ...
        for i in range(0, num_to_upload, upload_batch_size):
            ...
            for file_info in batch:
                ...
                upload_url = file_info.get("upload_url") or file_info.get(
                    "presigned_url"
                )
                if not isinstance(file_hash, str) or not isinstance(upload_url, str):
                    logger.warning(
                        f"Missing hash or upload URL in file info, skipping upload: {file_info}"
                    )
                ...
                if not filepath_str or not Path(filepath_str).exists():
                    logger.error(
                        f"Local file path missing or file not found for hash {file_hash}, skipping upload."
                    )
                    if isinstance(file_hash, str):
                ...
                upload_tasks.append(
                    self.upload_file(
                        upload_url, Path(filepath_str), file_hash=file_hash
                    )
                )
            if upload_tasks:
                logger.debug(
                    f"Attempting S3 upload sub-batch {i//upload_batch_size + 1}. Hashes: {hashes_in_batch}"
                )
                try:
                    batch_results = await asyncio.gather(
                    ...
                    logger.debug(
                        f"Completed S3 upload sub-batch for hashes: {hashes_in_batch}"
                    )
                    for idx, result in enumerate(batch_results):
                        ...
                        if isinstance(result, Exception):
                            ...
                            if reason == "executor_unavailable":
                                logger.info(
                                    f"Executor unavailable during upload of {current_hash}; aborting remaining uploads."
                                )
                                return {
                                    "success": False,
                                    "status": "upload_aborted_shutdown",
                                    "message": "Upload aborted due to shutdown",
                                    "uploaded": len(uploaded_hashes),
                                    "existing": existing_count,
                                    "failed": len(upload_failed_hashes) + 1,
                                    "failed_filenames": sorted(failed_filenames_set),
                                }

                            logger.error(
                                f"Failed to upload file {current_hash}: {type(result).__name__}: {result}"
                            )
                            exception_context = {
                            ...
                            logger.error(
                                f"Upload function returned False for file {current_hash}"
                            )
                            capture_media_message(
                    ...
                    logger.exception(
                        f"Unexpected error during asyncio.gather for S3 uploads (hashes {hashes_in_batch}): {e}"
                    )
                    capture_media_exception(
                        e,
                        context={
            ...
            if progress_callback:
                progress_value = 0.3 + (
                    0.6 * min(i + upload_batch_size, num_to_upload) / num_to_upload
                )
                progress_callback(progress_value)
        ...
        logger.info(
            f"Upload phase complete. Succeeded: {final_uploaded_count}, Failed during upload: {final_upload_failed_count}"
        )
        ...
        if uploaded_hashes_unique:
            try:
                logger.info(
                    f"Confirming {final_uploaded_count} uploaded files with server (Batch ID: {batch_id})..."
                )
                if progress_callback:
                    progress_callback(0.95)
                await self.confirm_media_bulk_upload(
                    batch_id, bulk_operation_id, uploaded_hashes_unique
                )
    ...
    @retry(max_tries=3, delay=1)
    async def confirm_media_bulk_upload(
        self, batch_id: str, bulk_operation_id: str, confirmed_files: List[str]
    ) -> None:  # Return None on success
        ...
        async with semaphore:
            ...
            try:
                ...
                logger.info(
                    f"Successfully confirmed {len(confirmed_files)} files for batch {batch_id}"
                )
                ...
                exc = MediaServerError(
                    f"Server error {status} confirming upload: {text}"
                )
                context = {
                    "stage": "confirm_media_bulk_upload",
                    "batch_id": batch_id,
                ...
                logger.exception(
                    f"Unexpected error during confirm_media_bulk_upload: {e}"
                )
                capture_media_exception(
                    e,
                    context={
                ...
                raise MediaServerError(
                    f"Unexpected error confirming upload: {e}"
                ) from e
