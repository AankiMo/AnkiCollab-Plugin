...
from ... import main
from ...media_progress_indicator import (
    show_media_progress,
    update_media_progress,
    complete_media_progress,
)
...
class Deck(JsonSerializableAnkiDict):
    ...
    def _load_deck_config(self):
        ...
        if conf_id:
            ...
            logger.warning(
                f"Deck {self.anki_dict.get('name', 'unknown')} has no config ID"
            )
            ...
            (
                {
                    ...
                    "deck_configurations": list(self.metadata.deck_configs.values()),
                }
    ...
    def get_media_file_list(self, data_from_models=True, include_children=True):
        ...
        for note in self.notes:
            ...
            if anki_object is None:
                continue
            join_fields = (
                anki_object.joined_fields
                if hasattr(anki_object, "joined_fields")
                else anki_object.joinedFields
            )
    ...
    def get_media_file_note_map(self, protected_fields, include_children=True):
        ...
        for note in self.notes:
            ...
            if not note_model:
                logger.warning(
                    f"Note model {note.note_model_uuid} not found in metadata, skipping note"
                )
                continue
            ...
            if not model_name:
                logger.warning(
                    f"Note model {note.note_model_uuid} has no name, skipping"
                )
                continue
        ...
        if include_children:
            for child in self.children:
                media_file_note_pairs.extend(
                    child.get_media_file_note_map(protected_fields, include_children)
                )
    ...
    def _get_media_from_models(self):
        ...
        for model in self.metadata.models.values():
            ...
            if model_id is not None:
                ...
                logger.warning(
                    f"Note model '{model_name}' has no ID, skipping for media extraction"
                )
    ...
    def _add_models_from_children(self, json_dict, note_models_list):
        note_models_list += [
            NoteModel.from_json(model) for model in json_dict.get("note_models", [])
        ]
    ...
    def calculate_total_work(self, include_children=True):
        ...
        if include_children:
            for child in self.children:
                child_note_count = child.calculate_total_work(
    ...
    def _start_media_download_from_main_thread(
        self, deck_hash, missing_files, media_result
    ):
            ...
            try:
                ...
                for i in range(0, len(missing_files), batch_size):
                    ...
                    def progress_callback(progress_ratio):
                        ...
                        overall_ratio = (
                            total_completed / len(missing_files)
                            if len(missing_files) > 0
                            else 1.0
                        )

                        aqt.mw.taskman.run_on_main(
                            lambda: update_media_progress(
                                overall_ratio, total_completed
                            )
                        )

                    batch_result = sync_run_async(
                        main.media_manager.get_media_manifest_and_download,
                        deck_hash=deck_hash,
                        filenames=batch,
                ...
                message = (
                    ...
                    else "All files were already present"
                )
                aqt.mw.taskman.run_on_main(
                ...
                try:
                    ...
                    logger.error(f"Failed to report to Sentry: {sentry_error}")
                media_result.update(
                    {"success": False, "message": f"Media download failed: {e}"}
                )
    ...
    def on_success_wrapper(self, result):
        ...
        if missing_files and deck_hash:
            ...
            self._start_media_download_from_main_thread(
                deck_hash, missing_files, media_result
            )
        else:
    ...
    def delete_empty_subdecks(self):
        logger.info(
            f"Trying to delete empty subdecks in deck {self.anki_dict.get('name', 'unknown')}"
        )
        ...
        for name, did in anki_decks.children(self.root_deck_id):
            try:
                if (
                    not anki_decks.is_filtered(did)
                    and anki_decks.card_count(did, include_subdecks=True) == 0
                ):
                    filtered_children = [
                        cid
                        for _, cid in anki_decks.children(did)
                        if anki_decks.is_filtered(cid)
                    ]
                    if filtered_children:
                        parent_name = anki_decks.immediate_parent(name)
                        parent_did = (
                            anki_decks.id_for_name(parent_name)
                            if parent_name
                            else self.root_deck_id
                        )
                        if parent_did:
        ...
        class UnifiedProgressTracker:
            ...
            def update_notes_progress(self, completed_notes, deck_name=""):
                ...
                if self.total_notes > 0:
                    scaled_value = int(
                        (completed_notes / self.total_notes) * self.NOTES_PORTION
                    )
                else:
                    scaled_value = 0
    ...
    def handle_notetype_changes(self, collection: Collection) -> bool:
        ...
        try:
            logger.info(
                f"Starting notetype management for {len(self.metadata.models)} models"
            )
            ...
            logger.info(
                "Notetypes created/updated, proceeding with existing note type changes"
            )
    ...
    def _handle_notetype_duplicates(
        self, collection: Collection, failed_operations: List[str]
    ) -> bool:
        """
        ...
        try:
            ...
            for note_model in self.metadata.models.values():
                ...
                if not remote_uuid:
                    logger.warning(
                        f"Notetype '{model_name}' has no UUID, skipping duplicate check"
                    )
                    continue
                ...
                if existing_models_with_name:
                    ...
                    for local_notetype in existing_models_with_name:
                        ...
                        if self._are_notetypes_compatible(note_model, local_notetype):
                            ...
                            logger.info(
                                f"Found compatible local notetype for '{model_name}', will merge"
                            )
                            break
                    ...
                    if compatible_notetype:
                        ...
                        logger.warning(
                            f"Found incompatible local notetype '{model_name}', will rename remote"
                        )
    ...
    def _create_and_update_notetypes(
        self, collection: Collection, failed_operations: List[str]
    ) -> bool:
        """
        ...
        if collection is None:
            failed_operations.append(
                "Collection is None - cannot create/update notetypes"
            )
        ...
        for note_model in self.metadata.models.values():
            try:
                ...
                if UUID_FIELD_NAME not in note_model.anki_dict:
                    ...
                    note_model.anki_dict[UUID_FIELD_NAME] = new_uuid
                    logger.info(
                        f"Adding UUID field to notetype '{model_name}', uuid: {new_uuid}"
                    )

                # Use the new save_to_collection method which handles everything and captures field mapping
                new_notetype_dict, field_mapping = note_model.save_to_collection(
                    collection
                )
                # Store field mapping for later use in note processing
                if field_mapping is not None:
                    logger.info(
                        f"Notetype '{model_name}' saved with field mapping: {field_mapping}"
                    )
                ...
                if new_notetype_dict is not None:
                    ...
                    if (
                        "id" not in note_model.anki_dict
                        or note_model.anki_dict["id"] is None
                    ):
                ...
                if (
                    ...
                    if saved_model and "id" in saved_model:
                        ...
                        logger.info(
                            f"Retrieved ID {saved_model['id']} for notetype '{model_name}'"
                        )
                    else:
                        logger.error(
                            f"Failed to get valid ID for notetype '{model_name}'"
                        )
                ...
                logger.error(
                    f"Failed to process notetype '{model_name}': {e}", exc_info=True
                )
                success = False
                try:
                    sentry_sdk.capture_exception(e)
    ...
    def _change_existing_note_types(
        self, collection: Collection, failed_operations: List[str]
    ) -> bool:
        """
        ...
        try:
            ...
            note_guid_to_target_uuid = {
                note.get_uuid(): note.note_model_uuid for note in all_notes
            }
            ...
            for i in range(0, len(note_guids), LARGE_BATCH_SIZE):
                ...
                query = (
                    f"SELECT id, mid, guid FROM notes WHERE guid IN ({placeholders})"
                )
                ...
                try:
                    ...
                    for note_id, current_mid, note_guid in notes_in_db:
                        ...
                        if not current_uuid:
                            ...
                            if current_model:
                                ...
                                logger.info(
                                    f"Added missing UUID {new_uuid} to notetype '{current_model.get('name', current_mid)}'"
                                )
                            else:
                                problem_notes.append(
                                    f"Note {note_guid}: current notetype {current_mid} not found in collection"
                                )
                                continue
                        ...
                        if not target_model_dict:
                            problem_notes.append(
                                f"Note {note_guid}: target model {target_uuid} not found"
                            )
                            continue
                        ...
                        if not target_mid:
                            problem_notes.append(
                                f"Note {note_guid}: target model {target_uuid} has no ID"
                            )
                            continue
                    ...
                    logger.warning(
                        f"Error processing note batch {i//LARGE_BATCH_SIZE + 1}: {e}"
                    )
                    continue

            logger.info(
                f"Found {len(note_type_changes)} different notetype changes to apply"
            )

            # todo report these issues to the user in a dialog
            if problem_notes:
                logger.warning(
                    f"Found {len(problem_notes)} problematic notes that were skipped"
                )
            ...
            for (old_mid, new_mid), note_ids in note_type_changes.items():
                ...
                try:
                    ...
                    if not old_model_dict or not new_model_dict:
                        logger.warning(
                            f"Could not find models for change {old_mid} -> {new_mid}"
                        )
                        success = False
                        continue

                    # Check if this change might cause data loss
                    requires_review = self._assess_change_risk(
                        old_model_dict, new_model_dict
                    )

                    if requires_review:
                        old_name = old_model_dict.get("name", "unknown")
                        new_name = new_model_dict.get("name", "unknown")
                        notes_requiring_review.append(
                            f"Notetype change '{old_name}' -> '{new_name}' ({len(note_ids)} notes)"
                        )

                    batch_success = self._apply_notetype_change_optimized(
                        collection, old_mid, new_mid, note_ids
                    )
                    if batch_success:
                        total_notes_changed += len(note_ids)
                        logger.info(
                            f"Successfully changed {len(note_ids)} notes from {old_model_dict.get('name')} to {new_model_dict.get('name')}"
                        )
                    else:
                        success = False
                    ...
                    failed_operations.append(
                        f"Notetype change {old_mid} -> {new_mid}: {e}"
                    )
                    logger.error(
                        f"Failed to change notetype for {len(note_ids)} notes: {e}",
                        exc_info=True,
                    )
                    try:
                        sentry_sdk.capture_exception(e)
            ...
            if total_notes_changed > 0:
                logger.info(
                    f"Successfully changed {total_notes_changed} notes to new notetypes"
                )

            if notes_requiring_review:
                logger.warning(
                    "MANUAL REVIEW REQUIRED for the following notetype changes:"
                )
                for review_note in notes_requiring_review:
                    logger.warning(f"  {review_note}")
                logger.warning(
                    "Please manually check these notes and consider reverting to backup if data is incorrect"
                )
            ...
            if len(old_fields) > 1 and len(new_fields) > 1:
                ...
                if len(common_fields) >= 2:
                    ...
                    position_changes = sum(
                        ...
                        if old_positions[field] != new_positions[field]
                    )
                    if (
                        position_changes > len(common_fields) // 2
                    ):  # More than half changed positions
                        return True
    ...
    def _apply_notetype_change_optimized(
        self, collection: Collection, old_mid: int, new_mid: int, note_ids: List[int]
    ) -> bool:
        """
        ...
        for i in range(0, len(note_ids), OPTIMIZED_BATCH_SIZE):
            ...
            try:
                batch_success = self._apply_notetype_change(
                    collection, old_mid, new_mid, batch
                )
                if not batch_success:
                    logger.warning(
                        f"Large batch {i//OPTIMIZED_BATCH_SIZE + 1} failed, trying smaller batches"
                    )
                    # Fallback to smaller batches if large batch fails
                    for j in range(0, len(batch), CHUNK_SIZE):
                        mini_batch = batch[j : j + CHUNK_SIZE]
                        try:
                            mini_success = self._apply_notetype_change(
                                collection, old_mid, new_mid, mini_batch
                            )
                            if not mini_success:
                                total_success = False
                        except Exception as e:
                            ...
                            try:
                                ...
                                logger.error(
                                    f"Failed to report to Sentry: {sentry_error}"
                                )
                            total_success = False
                else:
                    # Large batch succeeded
                    logger.debug(
                        f"Successfully processed large batch of {len(batch)} notes"
                    )
            except Exception as e:
                logger.error(
                    f"Exception in optimized batch {i//OPTIMIZED_BATCH_SIZE + 1}: {e}",
                    exc_info=True,
                )
                try:
                    sentry_sdk.capture_exception(e)
    ...
    def _apply_notetype_change(
        ...
        try:
            ...
            logger.info(
                f"Successfully changed {len(note_ids)} notes from '{old_name}' to '{new_name}'"
            )
            return True

        except Exception as e:
            logger.error(
                f"Failed to apply notetype change {old_mid} -> {new_mid}: {e}",
                exc_info=True,
            )
    ...
    def _validate_notetype_operations(
        self, collection: Collection, failed_operations: List[str]
    ) -> bool:
        """
            ...
            for note_model in self.metadata.models.values():
                ...
                if not actual_notetype:
                    ...
                    if UUID_FIELD_NAME not in actual_notetype:
                        ...
                        uuid_validation_failures.append(
                            f"{model_name} (UUID mismatch: expected {model_uuid}, got {actual_notetype.get(UUID_FIELD_NAME)})"
                        )

            if missing_notetypes:
                failed_operations.append(
                    f"Missing notetypes after operations: {missing_notetypes}"
                )
                logger.error(
                    f"Validation failed: missing notetypes {missing_notetypes}"
                )
                return False

            if uuid_validation_failures:
                failed_operations.append(
                    f"UUID validation failures: {uuid_validation_failures}"
                )
                logger.error(
                    f"Critical: UUID field validation failed for {uuid_validation_failures}"
                )
    ...
    def _are_notetypes_compatible(
        self, remote_model: NoteModel, local_notetype: Dict
    ) -> bool:
        """
    ...
    def _check_fields_compatible(
        self, remote_notetype: Dict, local_notetype: Dict
    ) -> bool:
        ...
        try:
            ...
            if is_compatible and len(local_fields) > len(remote_fields):
                ...
                logger.info(
                    f"Local notetype has {len(extra_fields)} additional fields: {list(extra_fields)} - this is compatible"
                )
    ...
    def save_metadata(self, collection: Collection, home_deck: Optional[str] = None):
        ...
        if not self.handle_notetype_changes(collection):
            logger.warning(
                "Some notetype operations failed, but continuing with deck import"
            )
    ...
    def save_decks_and_notes_bulk(
        self, collection, progress_tracker, import_config: ImportConfig
    ):
        """
        ...
        try:
            ...
            self._collect_all_notes(
                all_notes, note_to_deck_map, "", import_config.home_deck
            )
            ...
            root_deck_name = self._create_deck_structure(
                collection, "", import_config.home_deck, server_root_name
            )
            ...
            if temp_deck_id:
                self._cleanup_temp_deck(
                    collection, temp_deck_id, temp_deck_name, "success"
                )
                temp_deck_id = None  # Mark as cleaned up

            current_phase = "media_processing"
            media_files = self.get_media_file_list(
                data_from_models=True, include_children=True
            )

            if media_files:
                # Path traversal protection: only allow basenames without directory separators
                media_files = [
                    f for f in media_files if f == os.path.basename(f) and ".." not in f
                ]
            ...
            logger.error(
                f"Error in bulk import at phase '{current_phase}': {str(e)}",
                extra=error_context,
            )
            ...
            if temp_deck_id:
                try:
                    self._cleanup_temp_deck(
                        ...
                        temp_deck_id,
                        temp_deck_name,
                        f"finally_block_after_{current_phase}",
                    )
                except Exception as cleanup_error:
                    logger.error(
                        f"Failed to cleanup temp deck in finally block: {cleanup_error}",
                        exc_info=True,
                    )
                    try:
    ...
    def _collect_all_notes(
        self, all_notes, note_to_deck_map, parent_name, home_deck, server_root_name=None
    ):
        ...
        for note in self.notes:
            ...
            if note.get_uuid() in note_to_deck_map:
                logger.warning(
                    f"Note {note.get_uuid()} already mapped to deck {note_to_deck_map[note.get_uuid()]}, overwriting with {full_name}"
                )
            note_to_deck_map[note.get_uuid()] = full_name

        # Recursively process children, passing the server root name
        for child in self.children:
            child._collect_all_notes(
                all_notes, note_to_deck_map, full_name, None, server_root_name
            )
    ...
    def _get_full_deck_name(self, parent_name, home_deck, server_root_name=None):
        ...
        if parent_name and home_deck and server_root_name:
            ...
            if parent_name.startswith(home_deck):
                ...
                result = f"{parent_name}{self.DECK_NAME_DELIMITER}{current_deck_name}"
                logger.warning(
                    f"Unexpected subdeck mapping: {current_deck_name} -> {result}"
                )
                return result

        # Standard behavior for non-mapped scenarios
        result = (
            parent_name + self.DECK_NAME_DELIMITER if parent_name else ""
        ) + current_deck_name
        return result

    def _bulk_process_all_notes(
        ...
        has_separate_new_notes_deck = bool(
            import_config.new_notes_home_deck
            and import_config.new_notes_home_deck.strip()
            and import_config.new_notes_home_deck != import_config.home_deck
        )
        ...
        for note in all_notes:
            ...
            if note_model_uuid not in self.metadata.models:
                logger.warning(
                    f"Note model {note_model_uuid} not found in metadata, skipping note {note_uuid}"
                )
                continue
            ...
            if target_deck:
                if note_uuid in existing_note_set:
                    ...
                    if (
                        has_separate_new_notes_deck
                        and home_deck_prefix
                        and target_deck.startswith(home_deck_prefix)
                    ):
                        relative_path = target_deck[home_deck_prefix_len:].lstrip(
                            self.DECK_NAME_DELIMITER
                        )
                        if relative_path:
                            note_to_deck_map[note_uuid] = (
                                f"{import_config.new_notes_home_deck}{self.DECK_NAME_DELIMITER}{relative_path}"
                            )
                        else:
                            note_to_deck_map[note_uuid] = (
                                import_config.new_notes_home_deck
        ...
        logger.info(
            f"Created temporary deck '{temp_deck_name}' with ID {self.root_deck_id}"
        )

        processed_count = 0
        progress_update_interval = max(
            1, total_notes // 20
        )  # Update progress ~20 times total

        all_new_notes = []
        all_update_notes = []

        # Process all model groups
        all_model_uuids = set(notes_by_model_new.keys()) | set(
            notes_by_model_update.keys()
        )
        ...
        for note_model_uuid in all_model_uuids:
            try:
                ...
                if not note_model:
                    logger.warning(
                        f"Note model {note_model_uuid} not found in metadata, skipping notes"
                    )
                    continue
                ...
                logger.info(
                    f"Processing {total_model_notes} notes for model {note_model.anki_dict.get('name', 'unknown')} ({len(model_new_notes)} new, {len(model_update_notes)} updates)"
                )

                if model_new_notes:
                    self._batch_process_new_notes(
                        model_new_notes,
                        collection,
                        note_model,
                        field_mapping,
                ...
                if model_update_notes:
                    self._batch_process_update_notes(
                        model_update_notes,
                        collection,
                        note_model,
                        field_mapping,
                ...
                if (
                    ...
                    or processed_count == total_notes
                ):
        ...
        try:
            if all_new_notes:
                Note.bulk_add_notes(
                    collection, all_new_notes, self.root_deck_id, import_config
                )
            ...
            if all_update_notes:
                Note._bulk_update_notes_preserving_placement(
                    collection, all_update_notes, note_to_deck_map, import_config
                )
    ...
    def _cleanup_temp_deck(self, collection, temp_deck_id, temp_deck_name, context):
        ...
        try:
            ...
            if not deck_exists:
                logger.info(
                    f"Temp deck {temp_deck_name} (ID: {temp_deck_id}) already deleted (context: {context})"
                )
                return

            # Check how many cards are in the temp deck
            card_count = collection.decks.card_count(
                temp_deck_id, include_subdecks=True
            )
            ...
            if card_count > 0:
                ...
                logger.info(
                    f"Cleaning up empty temp deck {temp_deck_name} (context: {context})"
                )

            # Remove the temporary deck
            collection.decks.remove([temp_deck_id])
            logger.info(
                f"Successfully removed temp deck {temp_deck_name} (ID: {temp_deck_id})"
            )

        except Exception as e:
            logger.error(
                f"Error cleaning up temp deck {temp_deck_name} (context: {context}): {e}",
                exc_info=True,
            )
            try:
    ...
    def _batch_process_new_notes(
        ...
        self._batch_process_notes(
            ...
            int_time,
            is_new=True,
            existing_note_map=None,
        )

    def _batch_process_update_notes(
        self,
        notes_batch,
        collection,
        note_model,
        ...
        def process_single_note(note):
            ...
            if model_id is None or model_id == 0:
                ...
                if saved_model and saved_model.get("id"):
                    ...
                    logger.warning(
                        f"Had to fetch model ID {model_id} for note processing"
                    )
                else:
                    raise ValueError(
                        f"No valid model ID found for notetype {note_model.anki_dict.get('name', 'unknown')}"
                    )
        ...
        try:
            ...
            for note in notes_batch:
                try:
                    ...
                    logger.warning(
                        f"Error processing individual {note_type} note {note.get_uuid()}: {note_error}"
                    )
    ...
    def _restore_original_note_ids(self, collection, notes):
        ...
        notes_with_original_ids = [
            note
            for note in notes
            if hasattr(note, "_original_id") and note._original_id
        ]
        ...
        try:
            logger.info(
                f"Checking {len(notes_with_original_ids)} notes for safe ID restoration"
            )
            ...
            for note in notes_with_original_ids:
                if note._original_id in existing_ids:
                    ...
                    logger.warning(
                        f"Note ID {note._original_id} already exists, keeping new ID {note.anki_object.id} for safety"
                    )
                else:
            ...
            if not safe_notes:
                logger.info(
                    "No safe ID restorations possible - all original IDs are taken"
                )
            else:
                logger.info(
                    f"Restoring original creation timestamps for {len(safe_notes)} notes ({len(conflicted_notes)} skipped for safety)"
                )
                ...
                logger.info(
                    f"Successfully restored {len(safe_notes)} original creation timestamps"
                )
    ...
    def _create_deck_structure(
        self, collection, parent_name, home_deck, server_root_name=None
    ):
        ...
        try:
            ...
            full_name = self._save_deck(
                collection, parent_name, home_deck, server_root_name
            )

            # For child decks, we need to maintain the home deck context
            # but ensure proper relative structure mapping
            for child in self.children:
                child._create_deck_structure(
                    collection, full_name, None, server_root_name
                )

            return full_name
        except Exception as e:
            logger.warning(
                f"Error creating deck structure for {self.anki_dict.get('name', 'unknown')}: {e}"
            )
            return parent_name
    ...
    def _save_deck(self, collection, parent_name, home_deck, server_root_name=None):
        ...
        if not parent_name and home_deck:
            ...
            if not deck_dict:
                new_deck_id = collection.decks.id(full_name)
                deck_dict = collection.decks.get(
                    new_deck_id
