...
class AnkiJsonExporter(DeckExporter):
    ...
    def export_to_directory(
        ...
        deck_filename = deck_directory.joinpath(self.deck_file_name).with_suffix(
            DECK_FILE_EXTENSION
        )
    ...
    def _copy_media(self, deck, deck_directory):
        ...
        for file_src in deck.get_media_file_list():
            try:
                shutil.copy(
                    os.path.join(self.collection.media.dir(), file_src),
                    str(media_directory.resolve()),
                )
            except IOError as ioerror:
                print(
                    "Failed to copy a file {}. Full error: {}".format(file_src, ioerror)
                )
